...
/*   Updated: 2024/04/15 07:33:17 by srandria         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	formatter(&next_l, buff, buff_e, &bs_pos);
	while (bs_pos != -2)
	{
		bytes = read(fd, buff, BUFFER_SIZE);
		if (bytes == -1)
			return (free(buff_e), buff_e = NULL, free(buff), NULL);
		buff[bytes] = '\0';
		if (bs_pos > -1 || buff_e)
			if (trim_save(&next_l, &buff_e, &buff, &bs_pos) == 1)
				return (free(buff), buff = NULL, next_l);
		if (bytes == 0)
			break ;
		hybrid(&next_l, &buff, &buff_e, &bs_pos);
	}
